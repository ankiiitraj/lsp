    ...
    return (
        <div style={{ height: "100vh", width: "100vw", fontSize: "x-large", display: "flex", justifyContent: "center", alignItems: "center", color: "white  "}}>
            {(meetingId && !window.location.pathname.split('/')[2]) && 
                <>
                    <div>
                        {isThumbNailUploaded ? meetingId && <Link to={`/meeting/${meetingId}`}>Enter Meeting and start livestream</Link> : meetingId ? <ImageUploader meetingId={meetingId} setImgUploadedStatus={setThumbnailUploaded}/> : <>Something bad happened, try reloading</>}
                    </div>
                </>
            }
        </div>
